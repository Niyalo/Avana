...
const na = () => {
  ...
  const handlePopup = (e, username) =>{
    if(e.target.id === "popupOpener") setPopup(username)
  }

  const applicationList = applications? applications.map(items => (
    <div id='popupOpener' onClick={handlePopup(items.username)} className="cursor-pointer w-full px-8 py-4 border bg-white0 border-grey50 rounded-sm justify-start items-center gap-6 inline-flex hover:shadow-sm hover:bg-white50">
  ...
  return (
    <div className='adminContent flex flex-col gap-6'>
      ...
      <NewApplications username={popup} onClose={()=>setPopup("")} />
    </div>
  )
}
