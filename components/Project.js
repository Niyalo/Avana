...
const Project = ({isVisible, onClose, projectName}) => {
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
          try {
            ...
            const selectedProject = projects.filter((project) => project.project_name === `${projectName}`)
    ...
    const imageCarousel = projectData? projectData.sections.map(items => (
            <Image src={items.img} alt="section image" width={400} height={300} />
    )):null

    const sectionContent = projectData? projectData.sections.map(items => (
